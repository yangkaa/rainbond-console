...
function release_allinone() {
  ...
  docker build --network=host --build-arg VERSION="${VERSION}" --build-arg IMAGE_NAMESPACE="${IMAGE_NAMESPACE}"  --build-arg CLONE_URL="${CLONE_URL}" --build-arg RELEASE_DESC="${release_desc}" --build-arg ARCH="${ARCH}" -t "${imageName}" -f Dockerfile.allinone .
  if [ $? -ne 0 ]; then
    exit 1
  fi
  if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
